...
for (var i=0;i<mass.length;i++){
        arr.push(mass[i].price.newUan || mass[i].price);}
...
k.push(arr[i].replace("грн"," "));
m.push(k[i].replace(" ", ""));
}

for (var i=0;i<arr.length;i++){
        mainarr.push( parseInt(m[i]));
}

var newPrice;

for (var i=0;i<arr.length;i++){
mass[i].newPrice = mainarr[i];}
...
function compare(a,b){
      ...
      {
        console.log(`${array[i].newPrice} = ${array[i].name}`);
}

console.log(mainarr.sort(compareNumeric));
...
contentItem.className = "contentIteam";

let Name = document.createElement("div");
Name.innerHTML = array[i].name;

let Price = document.createElement("div");
if (array[i].price.newUan) {
  Price.innerHTML = "New price: " + array[i].price.newUan;
  var OldPrice = document.createElement("strike");
  OldPrice.innerHTML = "Old price: " + array[i].price.oldUan;
  flag = true;
} else {
...
Reviews.innerHTML = array[i].ratingRevievs;

contentItem.appendChild(Name); 
contentItem.appendChild(Price); 
if(flag) contentItem.appendChild(OldPrice);
contentItem.appendChild(Reviews); 
div.appendChild(content);
document.body.appendChild(div);

  document.body.style.cssText = `font-family: cursive;
  font-weight: 700;
  ...
  div.style.cssText=`display: flex; padding: 50px 0; justify-content: space-evenly;`;
        contentItem.style.cssText = `display: flex;
  ...
  OldPrice.style.cssText=`color: grey;
  font-weight: 300;`;
  
  Name.style.cssText=`padding: 0 10px;
  color: blue;
  margin-bottom: 5px;`;

  Price.style.cssText = `color: red;`;
}
